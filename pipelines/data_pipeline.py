...
from EDA.data_spiltter import SimpleTrainTestSplitStrategy
...
def data_pipeline(
    ...
    try:
        ...
        # Fill Gender column with mode (most common gender)
        gender_mode = df.groupBy('Gender').count().orderBy(F.desc('count')).first()[0]
        df = df.fillna({'Gender': gender_mode})
        logger.info(f"✓ Filled missing Gender values with mode: {gender_mode}")
        ...
        # Also drop outlier indicator columns
