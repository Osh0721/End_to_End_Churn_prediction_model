...
class ModelInference:
    ...
    def load_scaler(self, scaler_dir: str = 'artifacts/scale') -> None:
        ...
        Args:
            scaler_dir: Directory containing scaler artifacts
        """
        logger.info(f"\n{'='*50}")
        ...
        try:
            ...
            # For pandas/sklearn inference, create a MinMaxScaler with saved parameters
            if metadata['framework'] == 'pyspark' and not self.use_spark:
                # Create sklearn MinMaxScaler from PySpark scaler metadata
                ...
                logger.info(f"✓ Loaded scaler for columns: {self.scaler_columns}")
            else:
                logger.warning("⚠ Scaler format not compatible with current inference mode")
    ...
    def preprocess_input(self, data: Dict[str, Any]) -> pd.DataFrame:
        ...
        try:
            ...
            if self.encoders:
                ...
                for col, encoder_data in self.encoders.items():
                    if col in df.columns:
                        if isinstance(encoder_data, dict) and encoder_data.get('encoding_type') == 'one_hot':
                            ...
                            # Legacy label encoding (for backward compatibility)
                            original_value = df[col].iloc[0]
            ...
            if 'CreditScore' in df.columns:
                ...
                # CRITICAL: Keep CreditScore column for model compatibility
                # Model was trained with both CreditScore and CreditScoreBins columns
            ...
            if hasattr(self, 'scaler') and self.scaler is not None:
                ...
                columns_to_scale = self.scaler_columns
                
                # Check if all scaling columns are present
                available_scale_cols = [col for col in columns_to_scale if col in df.columns]
                if len(available_scale_cols) != len(columns_to_scale):
                    missing = set(columns_to_scale) - set(available_scale_cols)
                    logger.warning(f"  ⚠ Missing scaling columns: {missing}")
                
                if available_scale_cols:
                    # Apply scaling
                    original_values = df[available_scale_cols].iloc[0].to_dict()
                    df[available_scale_cols] = self.scaler.transform(df[available_scale_cols])
                    scaled_values = df[available_scale_cols].iloc[0].to_dict()
                    
                    for col in available_scale_cols:
                        logger.info(f"  ✓ Transformed '{col}': {original_values[col]} → {scaled_values[col]:.4f}")
            else:
                logger.warning("  ⚠ No scaler available - skipping scaling step")
            
            # CRITICAL: Reorder columns to match training data order
            # The model expects specific column order based on training
            expected_columns = [
