...
def setup_mlflow_autolog():
    """Setup MLflow autologging - metrics only, no model logging"""
    mlflow_config = get_mlflow_config()
    if mlflow_config.get('autolog', True):
        mlflow.sklearn.autolog(
            log_models=False,        # ← don't auto-log models (you do it manually)
            log_model_signatures=False,
            log_input_examples=False
        )
        logger.info("MLflow autologging enabled for scikit-learn (models excluded)")
